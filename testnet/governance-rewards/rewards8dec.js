...
const getVoters = async() => {
   ...
   // if voters' amount committed >= 1 push the voters to the voters array!
     await txnHistory.transactions.map(receiver=>{
          if((receiver['asset-transfer-transaction'].amount)/1000000 >=1){
                voters.push({
                    sender:receiver.sender,
                    amount:(receiver['asset-transfer-transaction'].amount)/1000000
                })
...
const draftTransaction = async (voters) => {
    ...
    voters.forEach((voter)=>{ 
            ...
            var govreward=(percentagegovreward/100)*govRewardsPool;
             ...
             votedamount=Math.floor(votedamount * 1000000)

             console.log("yieldly "+votedamount/1000000)
    ...
    console.log("total asset "+totalasset/1000000);
}
